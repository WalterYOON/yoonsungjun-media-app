...
const FinanceSplitView = ({ filteredItems, plans, onItemClick }) => {
    const categorizedItems = useMemo(() => ({ income: filteredItems.filter(i => i.type === 'income').sort((a, b) => new Date(b.datetime || 0) - new Date(a.datetime || 0)), expense: filteredItems.filter(i => i.type === 'expense').sort((a, b) => new Date(b.datetime || 0) - new Date(a.datetime || 0)) }), [filteredItems]);
    ...
    const FinanceCard = ({ item }) => { const plan = plans.find(p => p.id === item.planId); return (<div onClick={() => onItemClick(item)} className="bg-[#f0e9de] p-4 rounded-xl border border-[#d4c4ac] hover:border-[#a0714a] cursor-pointer transition-all group"><div className="flex justify-between items-start mb-2"><h4 className="font-bold text-[#42392e] text-sm group-hover:text-[#a0714a] transition-colors flex items-center gap-2">{item.title}{item.productUrl && (<a href={item.productUrl.startsWith('http') ? item.productUrl : `https://${item.productUrl}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-[#5d6a7a] hover:text-blue-300" title="제품 링크"><ExternalLink size={12} /></a>)}</h4><span className={`text-sm font-bold font-mono ${item.type === 'income' ? 'text-[#5d7a5d]' : 'text-[#9b4d4d]'}`}>{item.type === 'income' ? '+' : '-'}₩{Number(item.amount).toLocaleString()}</span></div><div className="flex items-center justify-between text-xs"><span className="text-[#857460]">{item.datetime || '-'}</span><span className="text-[#a89880] truncate max-w-[120px]">{plan?.topic || '-'}</span></div>{item.memo && (<p className="text-xs text-[#a89880] mt-2 line-clamp-1">{item.memo}</p>)}</div>); };
...
const FinanceSubTab = () => {
    ...
    const [subTab, setSubTab] = useState('all'); const [periodFilter, setPeriodFilter] = useState('all'); const [localSelectedDate, setLocalSelectedDate] = useState(formatDateLocal(new Date())); const [searchTerm, setSearchTerm] = useState(''); const [viewMode, setViewMode] = useState('split');
    const debouncedSearchTerm = useDebounce(searchTerm, 300);
    const financeItems = useMemo(() => managements.filter(m => m.category === 'finance'), [managements]);
    const filteredItems = useMemo(() => { const filterByPeriod = (items) => { if (periodFilter === 'all') return items; const selected = parseLocalDate(localSelectedDate); return items.filter(item => { if (!item.datetime) return false; const itemDate = new Date(item.datetime); if (periodFilter === 'day') return formatDateLocal(itemDate) === localSelectedDate; if (periodFilter === 'week') { const weekStart = new Date(selected); weekStart.setDate(selected.getDate() - selected.getDay()); const weekEnd = new Date(weekStart); weekEnd.setDate(weekStart.getDate() + 6); return itemDate >= weekStart && itemDate <= weekEnd; } if (periodFilter === 'month') return itemDate.getFullYear() === selected.getFullYear() && itemDate.getMonth() === selected.getMonth(); return true; }); }; return filterByPeriod(financeItems.filter(i => { const typeMatch = subTab === 'all' || i.type === subTab; const searchMatch = !debouncedSearchTerm || i.title?.toLowerCase().includes(debouncedSearchTerm.toLowerCase()); return typeMatch && searchMatch; })).sort((a, b) => new Date(b.datetime || 0) - new Date(a.datetime || 0)); }, [financeItems, subTab, periodFilter, localSelectedDate, debouncedSearchTerm]);
    const periodItems = useMemo(() => { if (periodFilter === 'all') return financeItems; const selected = parseLocalDate(localSelectedDate); return financeItems.filter(item => { if (!item.datetime) return false; const itemDate = new Date(item.datetime); if (periodFilter === 'day') return formatDateLocal(itemDate) === localSelectedDate; if (periodFilter === 'week') { const weekStart = new Date(selected); weekStart.setDate(selected.getDate() - selected.getDay()); const weekEnd = new Date(weekStart); weekEnd.setDate(weekStart.getDate() + 6); return itemDate >= weekStart && itemDate <= weekEnd; } if (periodFilter === 'month') return itemDate.getFullYear() === selected.getFullYear() && itemDate.getMonth() === selected.getMonth(); return true; }); }, [financeItems, periodFilter, localSelectedDate]);
    ...
    const handleExportCSV = () => { if (filteredItems.length === 0) { showToast('내보낼 데이터가 없습니다.', 'error'); return; } const csvData = filteredItems.map(item => { const plan = plans.find(p => p.id === item.planId); return { 날짜: item.datetime || '', 구분: item.type === 'income' ? '수입' : '지출', 항목명: item.title || '', 금액: item.amount || 0, 프로젝트: plan?.topic || '-', 메모: item.memo || '' }; }); const headers = Object.keys(csvData[0]).join(','); const rows = csvData.map(row => Object.values(row).map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n'); const csvContent = '\uFEFF' + headers + '\n' + rows; const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `재무내역_${formatDateLocal(new Date())}.csv`; link.click(); showToast('CSV 파일이 다운로드되었습니다.', 'success'); };
    const FinanceListView = () => (<div className="vj-card bg-[#faf6ef] rounded-2xl border border-[#d4c4ac] overflow-hidden"><div className="max-h-[600px] overflow-y-auto custom-scrollbar">{filteredItems.length === 0 ? (<div className="p-12 text-center"><DollarSign size={48} className="text-[#d4c4ac] mx-auto mb-4" /><p className="text-[#857460]">등록된 재무 내역이 없습니다.</p></div>) : (<table className="w-full"><thead className="bg-[#f0e9de] sticky top-0 z-10"><tr className="text-left text-xs text-[#857460]"><th className="px-5 py-3 font-bold">날짜</th><th className="px-5 py-3 font-bold w-[70px]">구분</th><th className="px-5 py-3 font-bold">항목명</th><th className="px-5 py-3 font-bold text-right">금액</th><th className="px-5 py-3 font-bold">연결 프로젝트</th><th className="px-5 py-3 font-bold">메모</th><th className="px-5 py-3 font-bold w-[60px] text-right">수정</th></tr></thead><tbody>{filteredItems.map(item => { const plan = plans.find(p => p.id === item.planId); return (<tr key={item.id} onClick={() => { setSelectedItems(prev => ({ ...prev, edit: item })); toggleModal('finance', true); }} className="border-t border-[#e8dcc8] hover:bg-[#f5f0e6] cursor-pointer group transition-all"><td className="px-5 py-3 text-xs text-[#a89880] font-mono">{item.datetime || '-'}</td><td className="px-5 py-3"><span className={`inline-block whitespace-nowrap text-[11px] px-2.5 py-0.5 rounded-full font-bold border ${item.type === 'income' ? 'bg-[#5d7a5d]/20 text-[#5d7a5d] border-[#5d7a5d]/30' : 'bg-[#9b4d4d]/20 text-[#9b4d4d] border-[#9b4d4d]/30'}`}>{item.type === 'income' ? '수입' : '지출'}</span></td><td className="px-5 py-3"><div className="flex items-center gap-2"><span className="text-sm font-bold text-[#42392e] group-hover:text-[#a0714a] transition-colors">{item.title}</span>{item.productUrl && (<a href={item.productUrl.startsWith('http') ? item.productUrl : `https://${item.productUrl}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-[#5d6a7a] hover:text-blue-400" title="제품 링크"><ExternalLink size={12} /></a>)}</div></td><td className={`px-5 py-3 text-sm font-bold font-mono text-right ${item.type === 'income' ? 'text-[#5d7a5d]' : 'text-[#9b4d4d]'}`}>{item.type === 'income' ? '+' : '-'}₩{Number(item.amount).toLocaleString()}</td><td className="px-5 py-3 text-xs text-[#857460] truncate max-w-[120px]">{plan?.topic || '-'}</td><td className="px-5 py-3 text-xs text-[#a89880] truncate max-w-[150px]">{item.memo || '-'}</td><td className="px-5 py-3 text-right"><button onClick={e => { e.stopPropagation(); setSelectedItems(prev => ({ ...prev, edit: item })); toggleModal('finance', true); }} className="p-1.5 text-[#857460] hover:bg-[#d4c4ac]/50 rounded transition-all opacity-0 group-hover:opacity-100"><Edit size={14} /></button></td></tr>); })}</tbody></table>)}</div></div>);
    return (<div className="space-y-4"><div className="grid grid-cols-3 gap-4"><div className="vj-card bg-[#faf6ef] rounded-2xl p-4 border border-[#d4c4ac]"><div className="flex items-center gap-2 mb-1"><DollarSign size={16} className="text-[#5d7a5d]" /><span className="text-xs text-[#857460]">총 수입</span></div><div className="text-xl font-black text-[#5d7a5d]">₩{totalIncome.toLocaleString()}</div></div><div className="vj-card bg-[#faf6ef] rounded-2xl p-4 border border-[#d4c4ac]"><div className="flex items-center gap-2 mb-1"><ShoppingBag size={16} className="text-[#9b4d4d]" /><span className="text-xs text-[#857460]">총 지출</span></div><div className="text-xl font-black text-[#9b4d4d]">₩{totalExpense.toLocaleString()}</div></div><div className="vj-card bg-[#faf6ef] rounded-2xl p-4 border border-[#d4c4ac]"><div className="flex items-center gap-2 mb-1"><Activity size={16} className={balance >= 0 ? 'text-[#a0714a]' : 'text-[#9b4d4d]'} /><span className="text-xs text-[#857460]">수지</span></div><div className={`text-xl font-black ${balance >= 0 ? 'text-[#a0714a]' : 'text-[#9b4d4d]'}`}>{balance >= 0 ? '+' : ''}₩{balance.toLocaleString()}</div></div></div><div className="vj-card bg-[#faf6ef] rounded-2xl p-4 border border-[#d4c4ac]"><div className="flex items-center gap-3 flex-wrap"><div className="flex-1 min-w-[200px] relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#a89880]" /><input type="text" placeholder="검색..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-[#f0e9de] border border-[#d4c4ac] rounded-xl text-sm text-[#42392e] placeholder-[#a89880] focus:outline-none focus:border-[#a0714a]" /></div><div className="flex p-1 bg-[#f0e9de] rounded-xl">{['all', 'month', 'week', 'day'].map(p => (<button key={p} onClick={() => setPeriodFilter(p)} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${periodFilter === p ? 'bg-[#a0714a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`}>{p === 'all' ? '전체' : p === 'month' ? '월' : p === 'week' ? '주' : '일'}</button>))}</div>{periodFilter !== 'all' && (<input type="date" value={localSelectedDate} onChange={e => setLocalSelectedDate(e.target.value)} className="px-4 py-2 bg-[#f0e9de] border border-[#d4c4ac] rounded-xl text-sm text-[#42392e]" />)}<div className="flex p-1 bg-[#f0e9de] rounded-xl">{['all', 'income', 'expense'].map(t => (<button key={t} onClick={() => setSubTab(t)} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${subTab === t ? (t === 'income' ? 'bg-[#5d7a5d] text-[#faf6ef]' : t === 'expense' ? 'bg-[#9b4d4d] text-[#faf6ef]' : 'bg-[#a0714a] text-[#faf6ef]') : 'text-[#857460] hover:bg-[#e8dcc8]'}`}>{t === 'all' ? '전체' : t === 'income' ? '수입' : '지출'}</button>))}</div><div className="flex p-1 bg-[#f0e9de] rounded-xl"><button onClick={() => setViewMode('split')} className={`p-2 rounded-lg transition-all ${viewMode === 'split' ? 'bg-[#a0714a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`} title="분할 뷰"><LayoutGrid size={16} /></button><button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-[#a0714a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`} title="리스트 뷰"><Columns size={16} /></button></div><button onClick={handleExportCSV} className="px-4 py-2 bg-[#f0e9de] border border-[#d4c4ac] text-[#42392e] font-bold rounded-xl text-xs flex items-center gap-2 hover:bg-[#e8dcc8] transition-all"><Download size={14} /> CSV</button><button onClick={() => { setSelectedItems(prev => ({ ...prev, edit: null })); toggleModal('finance', true); }} className="px-4 py-2 vj-btn-primary text-[#faf6ef] font-bold rounded-xl text-xs flex items-center gap-2"><Plus size={14} /> 등록</button></div></div>{viewMode === 'split' ? (<FinanceSplitView filteredItems={filteredItems} plans={plans} onItemClick={(item) => { setSelectedItems(prev => ({ ...prev, edit: item })); toggleModal('finance', true); }} />) : (<FinanceListView />)}</div>);
};
...
const ExternalManagementTab = () => {
    ...
    return (<div className="space-y-4"><div className="vj-card bg-[#faf6ef] rounded-2xl p-4 border border-[#d4c4ac]"><div className="flex items-center justify-between"><div className="flex items-center gap-4"><h2 className="text-xl font-black text-[#42392e] flex items-center gap-2"><Briefcase size={24} className="text-[#a0714a]" />외부관리</h2><div className="flex p-1 bg-[#f0e9de] rounded-xl"><button onClick={() => setActiveSubTab('finance')} className={`px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeSubTab === 'finance' ? 'bg-[#a0714a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`}><DollarSign size={16} /> 재무</button><button onClick={() => setActiveSubTab('inquiry')} className={`px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeSubTab === 'inquiry' ? 'bg-[#a0714a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`}><Mail size={16} /> 제휴/문의</button></div></div></div></div>{activeSubTab === 'finance' ? (<FinanceSubTab />) : (<InquirySubTab />)}</div>);
};

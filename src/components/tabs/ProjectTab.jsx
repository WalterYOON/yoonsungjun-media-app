                ...
                <>
                    ...
                    <div className="w-[440px] flex-shrink-0 bg-[#faf6ef] border-r border-[#d4c4ac] flex flex-col">
                        <div className="p-4 border-b border-[#d4c4ac] bg-[#f0e9de]">
                            ...
                            <div className="flex gap-1 mb-2">{[['all', '전체'], ['draft', '기획'], ['active', '제작중'], ['completed', '완료']].map(([key, label]) => (<button key={key} onClick={() => { setStatusFilter(key); setSelected(null); setShowCreate(false); }} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${statusFilter === key ? 'bg-[#a0714a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`}>{label}</button>))}</div>
                            <div className="flex gap-1 mb-2">{[['all', '전체'], ['main', '본편'], ['shorts', '숏츠']].map(([key, label]) => (<button key={key} onClick={() => { setTypeFilter(key); setSelected(null); setShowCreate(false); }} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${typeFilter === key ? 'bg-[#5d6a7a] text-[#faf6ef]' : 'text-[#857460] hover:bg-[#e8dcc8]'}`}>{label}</button>))}</div>
